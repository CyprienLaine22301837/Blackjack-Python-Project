...
Decks = {
  ...
  "player1": {'nom': 'player1', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]},  
  "player2": {'nom': 'player2', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
  "player3": {'nom': 'player3', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
  "player4": {'nom': 'player4', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
  "player5": {'nom': 'player5', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
  "player6": {'nom': 'player6', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
  "player7": {'nom': 'player7', 'score':"0", 'money':100, 'bet':0, 'Use':False, 'victories':0, 'fini':False, 'split':0, 'splitpasfini':[], 'cards': [[]]}, 
}
...
def split_a_pair(player, Decks, split_area):    
    if Decks[player][split_area][0][0]==Decks[player][split_area][1][0] and Decks[player]['split']<=3:
        temp=Decks.pop(player[split_area][1])
...
while play_check == True:
    ...
    for i in range(joueurs):
        ...
        if Decks["player"+str(i+1)]['cards'][0][0][0] == Decks["player"+str(i+1)]['cards'][0][1][0]:
            ...
            if yesorno == "y":
                split_a_pair("player"+str(i+1), Decks)
                print(Decks)
                Decks["player"+str(i+1)]['split'] = Decks["player"+str(i+1)]['split']+1
                Decks["player"+str(i+1)]['splitpasfini'].append(0)
                Decks["player"+str(i+1)]['splitpasfini'].append(1)
                Decks["player"+str(i+1)]['cards'][0] = hit(Decks["player"+str(i+1)]['cards'][0], jeu)
            ...
            if Decks["player"+str(i+1)]['fini'] == False:
                if Decks["player"+str(i+1)]['split'] == 0:
                    ...
                    if yesorno == "y":
                        Decks["player"+str(i+1)]['fini'] = True
                    yesorno=="n"
                    yesorno=input("Voulez vous tirer?. (y/n)?")
                    if yesorno=="y":
                        Decks["player"+str(i+1)]['cards'][0] = hit(Decks["player"+str(i+1)]['cards'][0], jeu)
                    if counter_check(Decks["player"+str(i+1)]['cards'][0]) > 21:
                                print("BURN "+str(Decks["player"+str(i+1)]['nom']))
